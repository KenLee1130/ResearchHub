...
| 路徑 | 內容 | Claude 可做的事 |
|---|---|---|
| `Journal/yyyy/MM/yyyy-MM-dd.md` | 每日日記，待辦格式 `- [ ] 文字`／完成 `- [x]`／已放棄 `- [-]`／已搬移 `- [>]`（帶 @due 的待辦每天由 app 自動搬到今天，舊副本標 `>`，當天子項留作進度紀錄） | 讀取分析；**不要**主動改寫歷史日記。`- [>]` 是搬移不是失敗，統計時忽略 |
| （待辦標記語法） | `- [ ] 文字 !high @from(7/5) @due(7/10) @est(3h) @line(A)`：`!high`/`!low` 優先級、`@due(M/d)` 到期日、`@from(M/d)` 開始日（每日搬移時項目跳到那一天的日記，之後逐日跟隨）、`@est(3h/45m/90)` 預估時長、`@line(名字)` 主線歸屬。白名單制，其他 @ 內容不受影響 | 排程時把 `!high` 與快到期的排前面、用 `@est` 配時段長度；週檢討按 line 分線統計 |
