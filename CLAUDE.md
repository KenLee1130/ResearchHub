...
| （待辦標記語法） | `- [ ] 文字 !high @from(7/5) @due(7/10) @every(mon,thu) @remind(7/20 09:00) @est(3h) @line(A)`：`@due` 每天播獨立副本直到到期（含）、`@from` 界定開始日（單獨用＝只在那天出現）、`@every` 每逢週幾出現、`@remind` 到時推播、`@est` 預估時長、`@line` 主線歸屬。白名單制，其他 @ 內容不受影響 | 排程時把 `!high` 與快到期的排前面、用 `@est` 配時段長度；週檢討按 line 分線統計 |
