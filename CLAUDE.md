...
| 路徑 | 內容 | Claude 可做的事 |
|---|---|---|
| `Journal/yyyy/MM/yyyy-MM-dd.md` | 每日日記，待辦格式 `- [ ] 文字`／完成 `- [x]`／已放棄 `- [-]`。帶 @due/@every 的待辦每天由 app **播出獨立副本**在當天日記——各天互相獨立，勾掉只代表那一天；同文字多日出現屬正常，不列入拖延統計（`- [>]` 為早期遺留標記，忽略即可） | 讀取分析；**不要**主動改寫歷史日記 |
| （待辦標記語法） | `- [ ] 文字 !high @from(7/5) @due(7/10) @every(mon,thu) @remind(7/20 09:00) @est(3h) @line(A)`：`@due` 每天播獨立副本直到到期（含）、`@from` 界定開始日（單獨用＝只在那天出現）、`@every` 每逢週幾出現、`@remind` 到時推播、`@est` 預估時長（日記裡渲染成蕃茄進度條，格數 = est ÷ 蕃茄鐘設定的專注分鐘）、`@pomo(n)` 任務自身已投入顆數（進度條 ＋/− 按鈕改的就是它，與蕃茄鐘統計無關）、`@line` 主線歸屬。白名單制，其他 @ 內容不受影響。編輯器裡打 /command 進入命令列（/todo 新增、/list 開任務總覽） | 排程時把 `!high` 與快到期的排前面、用 `@est` 配時段長度；週檢討按 line 分線統計 |
